...
/// channel schema.
std::string BuildFileDescriptorSet(const gp::Descriptor* toplevelDescriptor) {
  gp::FileDescriptorSet fdSet;
  std::queue<const gp::FileDescriptor*> toAdd;
  toAdd.push(toplevelDescriptor->file());
  std::unordered_set<std::string> seenDependencies;
  while (!toAdd.empty()) {
    const gp::FileDescriptor* next = toAdd.front();
    toAdd.pop();
...
int main(int argc, char** argv) {
  ...
  mcap::ChannelId ch_pose_id;
  {
    ...
    // auto fdSet = foxglove::BuildFileDescriptorSet(example::Pose::descriptor()).SerializeAsString();
    std::string msg = foxglove::BuildFileDescriptorSet(example::Pose::descriptor());
    mcap::Schema schema("example.pose", "protobuf", msg);
    writer.addSchema(schema);

    // choose an arbitrary topic name.
    mcap::Channel channel("pose", "protobuf", schema.id);
    writer.addChannel(channel);
    ch_pose_id = channel.id;

    printf("wrote descriptor to channel and added to writer");
  }
